...
- [ ] **TOP PRIORITY: Auto-polish on Send path (value-add bug).** `prompt_assist.py:380` checks `pa_config.get("llm_refine", False)` — but configure.py writes `refine_backend: "ollama"` (not `llm_refine: True`). Result: Send path emits raw stitched template ("building a website for both a tanning club. Additional context: ..."). Alex feedback 2026-04-24: "it literally just pastes exactly what I say this is retarded." Fix: change the check to `if backend == "ollama" or backend == "api" or pa_config.get("llm_refine", False)` so the selected refine backend runs on EVERY assembly, not just when user clicks Refine button. Expected behavior: Ollama rewrites raw stitched speech → polished natural prompt before paste. THIS is the feature's actual value-add; without it, v2 is just slow dictation. Test: speak "build a website for Boca Tanning Club", hit Paste, output should be polished ("I want to build a website for Boca Tanning Club..."), not raw stitching.
- [ ] **PR 1 — Finish live mic test of `feat/prompt-assist-v2` → merge → tag v4.4.0-beta1.** Re-run from source via `.\start.bat` with all 6 live-test fixes applied + the auto-polish fix above. Validate: Zira opener, beep cue, 2-slot Q&A (Task + Context only, NO Format), overlay with polished (not raw) prompt + 4 buttons visible (Cancel/Add/Refine/Paste), voice-confirm ("say send"), Escape cancels.
